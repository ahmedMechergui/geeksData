...
// a renderer2 must be created at the component to access its DOM
  addScriptCDN(LinkCDN: string, renderer2: Renderer2): void {
...
// a renderer2 must be created at the component to access its DOM
  addScripts(renderer2: Renderer2, ...scriptsNames: string[]): void {
  ...
  // a renderer2 must be created at the component to access its DOM
  addScriptCustom(renderer2: Renderer2, script: string): void {
    const s = renderer2.createElement('script');
    s.type = 'text/javascript';
    s.text = script;
    renderer2.appendChild(this._document.body, s);
